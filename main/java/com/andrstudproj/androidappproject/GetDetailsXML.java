...
class GetDetailsXML extends AsyncTask<String, Void, String> {
    //
    public CallbackListener listener = null;

    //
    public GetDetailsXML(CallbackListener response) {
        listener = response;
    }
    ...
    protected String doInBackground(String... strings) {
        //Define a String for hotel details to translate as the String passed to the parameter, with all spaces replaced...
        //...with "+" in order to be passed to the URL for the API to work
        String hotelDetailsToTranslate = (strings[0]).replace(" ", "+");
        //
        ...
        try {
            ...
            //Define the details XML string as the StringBuilder, cast to String
            String xmlStringDetails = getXMLResult("http://newjustin.com/translateit.php?action=xmltranslations&english_words="
                    + hotelDetailsToTranslate);
            ...
            xpp.setInput(new StringReader(xmlStringDetails));
    ...
    protected void onPostExecute(String details) {
        listener.onFinishResponse(details);
    }
